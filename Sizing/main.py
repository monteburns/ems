...
if __name__ == '__main__':
    ...
    model.Discharge_power = pe.Var(model.T, bounds=[0, 300])

    model.n_smr = pe.Var(within=pe.PositiveIntegers, initialize = 10, bounds=[1, 10])
    model.n_wind = pe.Var(within=pe.PositiveIntegers,  initialize = 10, bounds=[1, 50])
    model.n_solar = pe.Var(within=pe.PositiveIntegers, initialize = 10, bounds=[1, 100])
    ...
    def provide_demand(model, t):
        ...
        return (model.n_smr * smr.capacity + model.n_wind * wind.capacity * model.WindP[
            t] + model.n_solar * solar.capacity * model.SolarP[t] + model.HydrogenP[t] + model.Discharge_power[t]) >= model.Demand[t]
